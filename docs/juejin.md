...
**附:错误兜底演示**(模型未返回文本时的可读错误态)

![错误态演示](assets/error-state-panel.png)

**使用说明**:

- 以上均为暗色主题下的**真实界面截图**(非示意图),可直接用于掘金配图;
- 图①右侧可另加箭头高亮 ✨ 按钮;图②适合作为正文"对比效果"主图;
- 掘金编辑器支持直接上传本地图片(发布时上传 `docs/assets/` 下文件,替换相对引用即可);
- ⚠️ 图中的草稿文本来自演示会话,含较具体的业务内容("读写分离/分片再平衡/变更日志归档"),对外发布前可将其替换为更通用的示例(如"帮我写一个周报生成工具"),或保留原样由你决定。
